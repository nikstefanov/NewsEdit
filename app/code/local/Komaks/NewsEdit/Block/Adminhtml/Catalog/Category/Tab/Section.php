 ...
 */
/**
 * categories - section relation edit block
 *
 * @category    Komaks
 * @package     Komaks_NewsEdit
 * @author      Ultimate Module Creator
 */
class Komaks_NewsEdit_Block_Adminhtml_Catalog_Category_Tab_Section extends Komaks_NewsEdit_Block_Adminhtml_Section_Tree
{
    ...
    {
        parent::__construct();
        $this->setTemplate('komaks_newsedit/catalog/category/tab/sections.phtml');
    }
    ...
    /**
     * Return array with sections IDs which the category is linked to
     *
     * @access public
     * @return array
     * @author Ultimate Module Creator
    ...
    {
        if (is_null($this->_sectionsIds)) {
            $sections = Mage::helper('komaks_newsedit/category')
                ->getSelectedSections($this->getCategory());
            $ids = array();
    ...
    {
        return implode(',', $this->getSectionsIds());
    }
    ...
    {
        $root = $this->getRoot();
        if ($root && in_array($root->getId(), $this->getSectionsIds())) {
    ...
    protected function _getNodeJson($node, $level = 1)
    {
        ...
        if (in_array($node->getId(), $this->getSectionsIds())) {
            $item['checked'] = true;
        }
        return $item;
    }
    ...
    /**
     * Returns array with nodes those are selected (contain current section)
     *
     * @access protected
     * @return array
     * @author Ultimate Module Creator
    ...
    /**
     * Returns JSON-encoded array of section children
     *
     * @access public
     * @param int $sectionId
     * @return string
    ...
    {
        return $this->getUrl(
            '*/newsedit_section_catalog_category/sectionsJson',
            array('_current' => true)
        );
    }

    /**
     * Return distinct path ids of selected section
     *
     * @access public
    ...
    public function buildNodeName($node)
    {
        ...
        $result .= '<a target="_blank" href="'.
            $this->getUrl('adminhtml/newsedit_section/index', array('id'=>$node->getId(), 'clear'=>1)).
